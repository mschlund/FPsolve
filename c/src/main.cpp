...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()
    ...
    ( "free", "free semiring" )
    ( "lossy", "if used with --file: downward closure via Courcelle\nif used with --file and --file2: compare two input grammars for inequality via downward closure\nuse --refine n to fix refinement depth")
    ( "refine", po::value<int>(), "refinement depth for inequality check via downward closure; only available for --lossy --file --file2")
