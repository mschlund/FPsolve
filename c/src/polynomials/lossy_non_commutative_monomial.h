...
#include "lossy_non_commutative_polynomial.h"

class LossyNonCommutativeMonomial : public NonCommutativeMonomial<LossyFiniteAutomaton> {


public:

  LossyNonCommutativeMonomial(const NonCommutativeMonomial<LossyFiniteAutomaton>& m) {
    this->idx_ = m.idx_;
    this->variables_ = m.variables_;
    this->srs_ = m.srs_;
  }
   ...
   */
  LossyNonCommutativePolynomial replaceConstants(std::map<LossyFiniteAutomaton, VarId> &constantsToVariables) const {
      LossyNonCommutativePolynomial temp;
      ...
      if((idx_.size() == 1) && firstFactorIsSR && firstFactorIsEpsilon) {
          return LossyNonCommutativePolynomial::one();
      }
      ...
      if(firstFactorIsSR) {
          ...
          if(firstFactorIsEpsilon) {
              temp = LossyNonCommutativePolynomial(variables_.at(idx_.at(1).second));
              startIndex = 2;
          } else {
              temp = LossyNonCommutativePolynomial(constantsToVariables[srs_.at(idx_.at(0).second)]);
          }
      } else {
          temp = LossyNonCommutativePolynomial(variables_.at(idx_.at(0).second));
      }
      ...
      for(int i = startIndex; i < idx_.size(); i++) {
          ...
          if(idx_.at(i).first == SemiringType) {
              ...
              if(!(srs_.at(idx_.at(i).second) == LossyFiniteAutomaton::one())) {
                  temp *= LossyNonCommutativePolynomial(constantsToVariables[srs_.at(idx_.at(i).second)]);
              }
          } else {
   ...
   * terminals as factors), this will return LossyNonCommutativePolynomial::null().
   *
  ...
  LossyNonCommutativePolynomial binarize(std::map<std::string, VarId> &binarizationVariables,
      ...
      if(idx_.size() != variables_.size()) {
          return LossyNonCommutativePolynomial::null();
      }

      LossyNonCommutativePolynomial temp = LossyNonCommutativePolynomial::one();
      LossyNonCommutativePolynomial suffix;
      ...
      while(variablesProcessed < get_degree() - 1) {
          ...
          if(variablesProcessed == 0) {
              ...
              temp = LossyNonCommutativePolynomial(variables_.at(index));
              suffix = LossyNonCommutativePolynomial(variables_.at(index));
                    ...
                    */
              temp = LossyNonCommutativePolynomial(variables_.at(index)) * temp; // multiplication from left is relevant -> noncommutative!
              suffix = LossyNonCommutativePolynomial(variables_.at(index)) * suffix;
              ...
              if(binarizationVariables.find(suffix.string()) != binarizationVariables.end()) {
                  temp = LossyNonCommutativePolynomial(binarizationVariables[suffix.string()]);
              } else {
                  var = Var::GetVarId();
                  ...
                  temp = LossyNonCommutativePolynomial(var);
              }
          }
      ...
      return LossyNonCommutativePolynomial(variables_.at(0)) * temp;
  }
  ...
  bool findLengthOfDerivableStrings(std::map<VarId, unsigned long> &lengthOfShortestTerminal,
          std::map<VarId, LossyNonCommutativeMonomial> &productionsForShortestWords,
   ...
   * is correct.
   */
  LossyFiniteAutomaton shortestDerivableTerminal(std::map<VarId, LossyNonCommutativeMonomial> &productionsForShortestWords) const {
  ...
  /*
   * See LossyNonCommutativePolynomial::componentIsSquarable(..) for commentary.
   */
...
private:
   ...
   *
   */
  LossyNonCommutativePolynomial generateIntersectionMonomials(std::vector<unsigned long> &states,
          std::map<unsigned long, std::map<unsigned char, std::forward_list<unsigned long>>> &transitionTable,
    ...
    LossyNonCommutativePolynomial result = LossyNonCommutativePolynomial::null();
      ...
      if(idx_[monomialFactorIndex].first == SemiringType) { // we have a semiring element
          ...
          if(monomialFactorIndex == idx_.size() - 1) {
              ...
              if(srs_[idx_[monomialFactorIndex].second] == LossyFiniteAutomaton::one()) {
                  ...
                  if(reachable.count(targetState) != 0) {
                      result = LossyNonCommutativePolynomial(srs_[idx_[monomialFactorIndex].second]);
                  }
              }
              ...
              if(srs_[idx_[monomialFactorIndex].second] == LossyFiniteAutomaton::one()) {
                  ...
                  LossyNonCommutativePolynomial suffixPolynomial = LossyNonCommutativePolynomial::null();
                  for(auto &nextState: reachable) {
                      suffixPolynomial += generateIntersectionMonomials(states, transitionTable, nextState, targetState,
                                              statesToIndices, oldVariablesToIndices, newVariables, (monomialFactorIndex + 1));
                  }

                  result = LossyNonCommutativePolynomial(srs_[idx_[monomialFactorIndex].second]) * suffixPolynomial;
              }
          }
          ...
          // check if we hit the last factor of the monomial; we stop the recursion here
          if(monomialFactorIndex == idx_.size() - 1) {
              result = LossyNonCommutativePolynomial(newVariables[statesToIndices[currentState]][statesToIndices[targetState]]
              ...
              for(auto &nextState: states) {
                  ...
                  result += LossyNonCommutativePolynomial(newVariables[statesToIndices[currentState]][statesToIndices[nextState]]
                                [oldVariablesToIndices.at(variables_[idx_[monomialFactorIndex].second])]) *
